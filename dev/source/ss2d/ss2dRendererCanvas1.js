    ...
    this.Graphics = {
		...
		DrawScene: function () {
            ...
            try {
				...
				var ctxPanoramaLayer 	= this.canvasCtx[0];
				var ctxSpriteLayer 		= this.canvasCtx[1];
				var panorama 			= this.panorama;
				var viewport			= this.viewport;
				
				//// Panorama zeichnen bei Aktualisierung
				if (panorama.active == true) {
					if (panorama.updated === true) {
						this.ClearScene(0);
						if ((panorama.x + panorama.width) < this.width) {
							var multiply = Math.round(this.width / (panorama.width - panorama.x)) + 1;
							for (var i = 1; i <= multiply; i++) {
								ctxPanoramaLayer.drawImage (panorama.image, (panorama.x + (i * panorama.width)), panorama.y, panorama.width, panorama.height);
							}
						}
						...
						ctxPanoramaLayer.drawImage (panorama.image, panorama.x, panorama.y, panorama.width, panorama.height);
						this.panorama.updated = false;
					}
				}
				...
				if (this.sprites.length > 0) {
					for (var i = 0, length = this.sprites.length; i < length; i++) {
						...
						if (sprite.gameObjId != "" && sprite.alpha > 0) {
							...
							if (((sprite.gameObjId.position.x + sprite.gameObjId.width) - this.viewport.x) > 0 && ((sprite.gameObjId.position.y + sprite.gameObjId.height) - viewport.y) > 0
								&& (sprite.gameObjId.position.x - viewport.x) < this.width && (sprite.gameObjId.position.y - viewport.y) < this.height) {

								//// Transparenz im Sprite?
								if (sprite.alpha < 1)
									ctxSpriteLayer.globalAlpha = sprite.alpha;

								//// Animation aktiv?
								if (sprite.aniKey >= 0) {
									var animation = this.animations[sprite.aniKey];
									ctxSpriteLayer.drawImage(animation.image, (sprite.aniCStep * sprite.gameObjId.width), 0, sprite.gameObjId.width, sprite.gameObjId.height, (sprite.gameObjId.position.x - viewport.x), (sprite.gameObjId.position.y - viewport.y), sprite.gameObjId.width, sprite.gameObjId.height);
								}
								else
									ctxSpriteLayer.drawImage(sprite.image, (sprite.gameObjId.position.x - viewport.x), (sprite.gameObjId.position.y - viewport.y), sprite.gameObjId.width, sprite.gameObjId.height);

								ctxSpriteLayer.globalAlpha = "1.0";
							}
						}
