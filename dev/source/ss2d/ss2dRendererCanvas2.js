...
function cRendererPlugin ()  {
    ...
    this.Graphics = {
		...
		Init: function() {
            ...
            var objCvSub = document.createElement("canvas");
            objCvSub.setAttribute("id", "SS2D_CANVAS_SUB");
            objCvSub.setAttribute("height", this.height);
            objCvSub.setAttribute("width", this.width);
			objCvSub.style.backgroundColor = "transparent";
			objCvSub.style.position = "absolute";
			objCvSub.style.left = "0px";
			objCvSub.style.top = "0px";
			this.drawCage.appendChild(objCvSub);
            ...
            if ($.browser.msie) {
                ...
                G_vmlCanvasManager.initElement(objCvSub);
            ...
            this.canvasCtx[1]   = objCvSub.getContext("2d");
            this.canvasCtx[2]   = objCv.getContext("2d");
            this.canvasCtx[100] = objCvFont.getContext("2d");
			
			this.canvasList.push("SS2D_CANVAS_BG");
			this.canvasList.push("SS2D_CANVAS_SUB");
            ...
            Update: function () {
                ...
                for (var i = ss2d.Graphics.aniSprites.length; i--;) {
                    if (ss2d.Graphics.aniSprites[i].objId.aniTick >= ss2d.Graphics.aniSprites[i].objId.aniWait) {
						...
						ss2d.Graphics.aniSprites[i].objId.updated = true;
		...
		/* /////////////////////////////////////////////////////////////////////////
         * @Func:   ClearSceneArea (ctxLayer, x, y, width, height)
         * @Author: Skarge
         * @Date:   2011/01/13
         * @Mod.:   -
         * @Desc:   Leeren der angegebenen Darstellungsflaeche
		 */
		ClearSceneArea: function (ctxLayer, x, y, width, height) {
	
			try {
				if (ctxLayer >= 0)
					this.canvasCtx[ctxLayer].clearRect(x, y, width, height);
				else
					throw "Kein Layer angegeben.";
			}
			catch (e) {
            ...
            try {
				...
				if (this.sprites.length > 0) {
					for (var i = 0, length = this.sprites.length; i < length; i++) {
						...
						var gameObj = sprite.gameObjId;
						if (gameObj != "" && sprite.alpha > 0 && sprite.updated == true && gameObj.layer != 0) {
							//// Sprite sichtbar oder ausserhalb der Anzeige?
							if (((gameObj.position.x + gameObj.width) - this.viewport.x) > 0 && ((gameObj.position.y + gameObj.height) - this.viewport.y) > 0
								&& (gameObj.position.x - this.viewport.x) < this.width && (gameObj.position.y - this.viewport.y) < this.height) {

								//// Anzeigebereich des Sprites leeren
								this.ClearSceneArea(gameObj.layer, sprite.old_x, sprite.old_y, gameObj.width, gameObj.height);

								//// Transparenz im Sprite?
								if (sprite.alpha < 1)
									this.canvasCtx[gameObj.layer].globalAlpha = sprite.alpha;

								//// Animation aktiv?
								if (sprite.aniKey >= 0) {
									var animation = this.animations[sprite.aniKey];
									this.canvasCtx[gameObj.layer].drawImage(animation.image, (sprite.aniCStep * gameObj.width), 0, gameObj.width, gameObj.height, (gameObj.position.x - this.viewport.x), (gameObj.position.y - this.viewport.y), gameObj.width, gameObj.height);
								}
								else
									this.canvasCtx[gameObj.layer].drawImage(sprite.image, (gameObj.position.x - this.viewport.x), (gameObj.position.y - this.viewport.y), gameObj.width, gameObj.height);

								this.canvasCtx[gameObj.layer].globalAlpha = "1.0";
								sprite.old_x   = gameObj.position.x;
								sprite.old_y   = gameObj.position.y;
